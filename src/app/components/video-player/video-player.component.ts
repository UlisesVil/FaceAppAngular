...
export class VideoPlayerComponent implements OnInit, OnDestroy{
  ...
  filters = [
    // {
    //   type: 'question',
    //   question: '¿Estas viendo este letrero? <b>Quien eres?</b>'
    // }
    {
      type: 'image',
      image: '02.png'
  ...
  listenerEvents=()=>{
   ...
   const  observer2$ = this.videoPlayerService.cbAi
    .subscribe(({ resizedDetections, displaySize, expressions, eyes})=>{
      //console.log({resizedDetections, displaySize, expressions, eyes});
  ...
  drawFace = ( resizedDetections, displaySize, eyes ) =>{
    ...
    this.overCanvas.getContext('2d').clearRect(0, 0, displaySize.width, displaySize.height); //con esto limpiamos el canvas (imagen transparente) donde van los puntos
